...
/usr/bin/xargs -a jre_modules.out /usr/lib/jvm/java-${JAVA_VERSION}-openjdk/bin/jlink --compress=zip-9 --no-header-files --no-man-pages --strip-debug --output=/opt/teragrep/${COMPONENT_NAME}/ --add-modules;
